// Cloudflare Pages counterpart to netlify/functions/yahoo.mts (and the Vite dev
// proxy in vite.config.ts). The browser hits /yahoo/... and this Function forwards
// it to Yahoo Finance server-side, so the browser never touches
// query1.finance.yahoo.com directly (it sends no CORS headers). quoteSummary is
// crumb-gated and gets the cookie/crumb dance; every other path is a plain
// pass-through. Without this, Cloudflare serves the SPA index.html for /yahoo/*
// and the browser fails to parse HTML as JSON.
